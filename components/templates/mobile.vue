<template>
	...
	>
		...
		<!-- <dr-a-float-btn-down
			v-if="
				['extra-large', 'large-screen', 'large', 'medium'].includes(
					width
				)
			"
			@click-down-btn="next"
...
export default {
	...
	methods: {
		swipe(direction) {
			...
			if (this.page === 4 && direction === "Left") {
				return this.next();
			}

			if (this.page === 4 && direction === "Right") {
				return this.prev();
			}

			// Page 5

			if (this.page === 5 && direction === "Left") {
				return this.page = 1
			}

			if (this.page === 5 && direction === "Right") {
				return this.prev();
			}
		},
